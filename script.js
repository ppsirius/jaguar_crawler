$( document ).ready(function() {
  ...
  $( "header" ).append( "<p>" + allImgArray + "</p>" );

  /** Rename name files
      You get file name jaguar_baner_1_1366x380.jpg 
      You can change var model, site or add baner text 
  **/
  ...
  while(i < link_name.length) {
    ...
    if(current_link.slice(7,22) === last_link) {
      // var new_link_string = model + '_' + baner + counter + '_' + resolution;
    ...
    last_link = current_link.slice(7,22);
    ...
    while(counter_mv < link_name.length) {
      ...
      $( "header" ).append( '<div> mv ' + mv_comends + ';</div>' );
      counter_mv++
    };
});
